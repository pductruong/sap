...
const char* ssid = "P519 5G";
const char* password = "deocomatkhau";

WebServer server(80);

const char* thingSpeakAPIKey = "U08SDHAU7OJX8DLU";  // Replace with your ThingSpeak Write API Key
...
String generateHTML(float temperature, float humidity, int soilMoisture, int rainPercent, int lightLevel) {
  ...
  html += "<div class='card temperature'><div class='card-header'></div><div class='icon'><i class='fas fa-thermometer-half'></i></div><div class='value'>" + String(temperature, 2) + " &deg;C</div><div class='label'>Temperature</div><div class='status'><div class='status-dot " + tempColor + "'></div><div>" + tempStatus + "</div></div></div>";
  html += "<div class='card humidity'><div class='card-header'></div><div class='icon'><i class='fas fa-tint'></i></div><div class='value'>" + String(humidity, 2) + "%</div><div class='label'>Humidity</div><div class='status'><div class='status-dot " + humColor + "'></div><div>" + humStatus + "</div></div></div>";
...
void sendDataToThingSpeak() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpResponseCode > 0) {
      ...
      Serial.println("Error sending data to ThingSpeak. Code: " + String(httpResponseCode));
    }
    http.end();
  }
}
...
void loop() {
  ...
  if (currentTime - lastThingSpeakSendTime >= thingSpeakInterval) {
